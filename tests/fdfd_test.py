...
@pytest.mark.parametrize(
    ...
    [
        ((5, 5, 5), 0.5, 4, (4, -16, 1), [0, 2, 10, 40, 100, 1000, 400, 203, 314],
...
@pytest.mark.parametrize(
    ...
    [
        ((5, 5, 5), 0.5, EPS_10_10_10, 4, (4, -16, 1), [0, 2, 10, 40, 100, 1000, 400, 203, 314],
...
def test_mat_selected_indices(size: Size, spacing: Size, eps: Union[float, np.ndarray],
                              ...
                              expected_db_data: np.ndarray, expected_db_indices: np.ndarray):
    grid = FDFD(size, spacing, eps=eps, pml=pml, pml_params=pml_params)
    actual_mat = grid.mat
...
@pytest.mark.parametrize(
    ...
    [
        ((10, 10), 1, EPS_10_10, 4, (4, -16, 1), [0, 2, 10, 40, 100, 130, 190, 80],
         ...
         [0, 9, 1, 7, 10, 16, 28, 6]),
        ((20,), 2, EPS_10, 4, (4, -16, 1), [0, 2, 4, 6, 8],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((5,), 0.5, EPS_10, 4, (4, -16, 1), SOURCE_10,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((5,), 0.5, EPS_10, 4, (4, -16, 1), np.ones(30)),
    ],
)
def test_solve_src_size_error(size: Size, spacing: Size, eps: Union[float, np.ndarray],
    ...
    with pytest.raises(ValueError, match='Expected src.size == '):
        FDFD(size, spacing, eps=eps, pml=pml, pml_params=pml_params).solve(src)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((5,), 0.5, EPS_10, 4, (4, -16, 1), SOURCE_10, True),
        ((3, 3), 1, EPS_3_3, None, (4, -16, 1), SOURCE_3_3, False),
        ((5,), 0.5, EPS_10, 4, (4, -16, 1), SOURCE_10, False)
    ]
)
