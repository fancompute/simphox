...
from simphox.sim import SimGrid, SimCrossSection
from simphox.grid import Port
from simphox.typing import Shape, Dim, Dim2, Dim3, Dim4, Union, Optional, List, MeasureInfo
...
@pytest.mark.parametrize(
    "shape, spacing, eps, port_list, fields, measure_info, profile_size_factor, expected_params",
    [
        ...
        ((40, 40, 30), 0.5, EPS_40_40_30, [(7, 7, 0, 2, 7, 3), (13, 13, 0, 2, 8, 3)],  # x,y,a,w,z,h
         SOURCE_40_40_30, None, 2,
         [[2.702992 + 2.683246j, 3.689899 + 4.437999j], [-1.134477 - 1.556019j, -0.764333 - 0.596152j]])
    ]
)
def test_measure_fn(shape: Shape, spacing: Dim, eps: Union[float, np.ndarray],
                    port_list: List[Dim4], fields: np.ndarray, measure_info: MeasureInfo, profile_size_factor: float,
                    expected_params: np.ndarray):
    grid = SimGrid(shape, spacing, eps=eps)
...
@pytest.mark.parametrize(
    "shape, spacing, eps, pml, pml_params, port_list,"
    ...
    [
        ...
        ((40, 40, 30), 0.5, EPS_40_40_30, None, (4, -16, 1),
         [(7, 7, 0, 2, 7, 3), (13, 13, 90, 2, 8, 4)], SOURCE_40_40_30, None, 2, [(7, 7, 7), (13, 13, 8)],
         [(0, 4, 6), (4, 0, 8)], [[0.030234, 0.053373], [0.026255, 0.044831]])
    ]
)
def test_port_modes(shape: Shape, spacing: Dim, eps: Union[float, np.ndarray], pml: Shape, pml_params: Dim3,
                    port_list: List[Dim4], fields: np.ndarray, measure_info: MeasureInfo, profile_size_factor: float,
                    expected_center: List[Dim3], expected_size: List[Dim3], expected_mode_mean_std: List[Dim2]):
    grid = SimGrid(shape, spacing, eps=eps, pml=pml, pml_params=pml_params)
    grid.port = {i: Port(*port_tuple) for i, port_tuple in enumerate(port_list)}
